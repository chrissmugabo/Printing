...
function getPDFPrintSettings() {
  ...
  option.marginsType = parseInt(
    marginsSetting.options[marginsSetting.selectedIndex].value
  );

  option.printBackground = document.getElementById("print-background").checked;
  option.printSelectionOnly =
    document.getElementById("print-selection").checked;
  return option;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  print_win.webContents.on("did-finish-load", function () {
    ...
    document
      .getElementById("save_pdf_button")
      .addEventListener("click", savePDF);
    document
      .getElementById("view_pdf_button")
      .addEventListener("click", viewPDF);
  });
